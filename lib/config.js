...
function readDefaultConfig() {
    try {
        return deepMerge(FALLBACK_CONFIG, JSON.parse(
            fs.readFileSync(PATHS.defaultConfigFile, 'utf8')
        ))
    } catch (error) {
        logger?.warn?.(`[bottle-plugin] 读取默认配置失败，使用内置默认值: ${error}`)
